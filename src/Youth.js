...
class Youth extends React.Component {
  ...
  render() {
    ...
    const mortalZeroNJData = this.state.mortalZeroNJData.map(([x, y]) => [
      ((x - this.state.lowDate) / this.state.xAxis) * lastWidth,
      scale(y, true, true) * 150
    ]);
    const mortalZeroNJDataAge = this.state.mortalZeroNJDataAge.map(map);
    const mortalFiftyNJDataAge = this.state.mortalFiftyNJDataAge.map(map);
    ...
    const yearlyZeroNJDataAge = this.state.yearlyZeroNJDataAge.map(map);
    const yearlyFiftyNJDataAge = this.state.yearlyFiftyNJDataAge.map(map);
    ...
    const yearlyOneTenNJDataAge = this.state.yearlyOneTenNJDataAge.map(map);
