...
class CE extends React.Component {
      ...
      if (actualDeaths) {//1920
        ...
        actData.push([x.year, actualDeaths.deaths]);
          ...
          if (thisdecade) {
            ...
            excessLife.push(
              thisdecade.pop - (birthd ? birthd.births * 1000 : 0) - deaths[1]
            );
            excessLifeData.push([
              x.year,
              thisdecade.pop - (birthd ? birthd.births * 1000 : 0) - deaths[1]
  ...
  render() {
    ...
    const excessLifeData = this.state.excessLifeData.filter(
    ...
    const excessLifeDat = excessLifeData.map(([x, y]) => [
      ((x - this.state.lowDate) / this.state.xAxis) *
        this.props.lastWidth *
        0.9,
      ((y - this.state.lowExp) / this.state.maxLife) * 150
    ]);
    //console.log(excessLifeDat);
